...
def reg(u: UserCreate):
    db = SessionLocal()
    try:
        if db.query(U).filter(U.username == u.username).first():
...
def login(f: OAuth2PasswordRequestForm = Depends()):
    db = SessionLocal()
    user = db.query(U).filter(U.username == f.username).first()
    if not user or user.p_hash != h_pw(f.password):
