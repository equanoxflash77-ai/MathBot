import os, re, hashlib, uvicorn, threading, nest_asyncio, time, asyncio
...
def start_app(stop_event):
    config = uvicorn.Config(app, host="0.0.0.0", port=8000, log_level="info")
    server = Server(config=config)
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    
    async def serve_with_check():
    ...
    loop.run_until_complete(serve_with_check())
    loop.close()
...
server_thread = threading.Thread(target=start_app, args=(stop_event,), daemon=True)
server_thread.start()
print("SERVER LIVE")
